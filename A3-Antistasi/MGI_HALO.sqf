...
_pole removeAction enable_halo;
_pole addAction ["Book <t color='#00d486'>Skydiving</t> group <t color='#ffc400'>500€</t>",{
	if (captive player) then {
		if((player getVariable "dinero") >= 500) then {
			player setVariable ["dinero",(player getVariable "dinero")-500,true];
		...
		hint "Must be undercover to take part in civilian tourist activities.";
	}
},[_jump_alt,_jump_safety], 5,false, true,"","vehicle _this == _this"];

_pole addAction ["Book <t color='#00d486'>Skydiving</t> solo <t color='#ffc400'>100€</t>",{
	if (captive player) then {
